...
function getGraphEdgesOfType_unstable<
	TGraph extends GraphWithTypeForEdge,
	TTypes extends TypesContainerForEdge<TGraph>,
>(
	graph: TGraph,
	...
	const args = [nodeOrTypes, neighborOrTypes, types] as const
	if (
		args
		...
		return getGraphEdgesOfTypeSingle(
			//@ts-expect-error Graphology library should throw if array is empty.
			graph,
			...args
		) as GetGraphEdgesOfTypesResult<TGraph, TTypes>
	}

	const [strArgs, [arrOrObj]] = partition(
		args,
		(arg) => typeof arg === "string"
	) as [[string] | [string, string], [TTypes]]

	if (Array.isArray(arrOrObj)) {
		for (const type of arrOrObj) {
			edges[type] = getGraphEdgesOfTypeSingle(graph, ...strArgs, type)
		}
		return edges
	}

	for (const type in arrOrObj) {
		edges[type] = getGraphEdgesOfTypeSingle(graph, ...strArgs, type)
	}
	return edges
}
