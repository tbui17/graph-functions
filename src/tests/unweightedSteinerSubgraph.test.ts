...
import { SteinerSubgraphError, unweightedSteinerSubgraph } from ".."
...
it("should exclude table2 in the subgraph", () => {
	...
	const res = unweightedSteinerSubgraph(graph, inp)
...
it("should keep attributes", () => {
	const inp = ["table1", "table3", "table5"]
	const graph2 = unweightedSteinerSubgraph(graph, inp)
...
it("should throw if nodes have no connection in the graph", () => {
	...
	expect(() => unweightedSteinerSubgraph(graph, inp)).toThrowError(
		SteinerSubgraphError
	)
})
